...
(*
let getConfigSection2<'T> fileName directory section =
    let y = getConfig2 fileName directory
    let z = y.GetSection(section)
    y |> fun x -> x.GetSection(section).Value |> JsonConvert.DeserializeObject<'T>

let getConfigSection<'T> = getConfigSection2<'T> None None
*)
